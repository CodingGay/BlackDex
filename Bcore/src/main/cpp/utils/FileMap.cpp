...
FileMap::~FileMap(void)
{
    ...
    if (mBasePtr && munmap(mBasePtr, mBaseLength) != 0) {
        //chensenhua    ALOGD("munmap(%p, %zu) failed\n", mBasePtr, mBaseLength);
    }
#endif
}
        ...
        bool readOnly)
{
    ...
    if (mPageSize == -1) {
        ...
        if (mPageSize == -1) {
            //chensenhua   ALOGE("could not get _SC_PAGESIZE\n");
            return false;
        }
    }
    ...
    if (ptr == MAP_FAILED) {
        //chensenhua  ALOGE("mmap(%lld,%zu) failed: %s\n",
        //chensenhua     (long long)adjOffset, adjLength, strerror(errno));
        return false;
    }
    mBasePtr = ptr;
    ...
    //chensenhua  ALOGV("MAP: base %p/%zu data %p/%zu\n",
    //chensenhua     mBasePtr, mBaseLength, mDataPtr, mDataLength);
...
int FileMap::advise(MapAdvice advice)
{
    ...
    if (cc != 0)
        //chensenhua  ALOGW("madvise(%d) failed: %s\n", sysAdvice, strerror(errno));
    return cc;
}
