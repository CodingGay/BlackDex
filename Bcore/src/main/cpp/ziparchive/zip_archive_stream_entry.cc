...
const std::vector<uint8_t>* ZipArchiveStreamEntryUncompressed::Read() {
  ...
  if (!archive->mapped_zip.ReadAtOffset(data_.data(), bytes, offset_)) {
    if (errno != 0) {
      //chensenhuaALOGE("Error reading from archive fd: %s", strerror(errno));
    } else {
      //chensenhuaALOGE("Short read of zip file, possibly corrupted zip?");
    }
    length_ = 0;
    return nullptr;
  }
...
bool ZipArchiveStreamEntryCompressed::Init(const ZipEntry& entry) {
  ...
  if (zerr != Z_OK) {
    if (zerr == Z_VERSION_ERROR) {
      //chensenhuaALOGE("Installed zlib is not compatible with linked version (%s)", ZLIB_VERSION);
    } else {
      //chensenhuaALOGE("Call to inflateInit2 failed (zerr=%d)", zerr);
    }
...
const std::vector<uint8_t>* ZipArchiveStreamEntryCompressed::Read() {
  ...
  while (true) {
    if (z_stream_.avail_in == 0) {
      ...
      if (!archive->mapped_zip.ReadAtOffset(in_.data(), bytes, offset_)) {
        if (errno != 0) {
          ...
          //chensenhua ALOGE("Short read of zip file, possibly corrupted zip?");
        }
        return nullptr;
      }
    ...
    if (zerr != Z_OK && zerr != Z_STREAM_END) {
      //chensenhua ALOGE("inflate zerr=%d (nIn=%p aIn=%u nOut=%p aOut=%u)", zerr, z_stream_.next_in,
      //chensenhua     z_stream_.avail_in, z_stream_.next_out, z_stream_.avail_out);
      return nullptr;
    }
